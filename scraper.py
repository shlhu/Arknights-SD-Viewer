...
# Usage: python3 scraper.py operatorinternalname targetchar enemyflag<Y/N> nodormflag<Y/N> nobackflag<Y/N> 
...
internalname = sys.argv[1] # Operator Internal Name. Used within Arknights codebase and filepaths. See charData.js first level keys.
charname = sys.argv[2]

options = ["front", "back", "base"]
if (len(sys.argv) > 3 and sys.argv[3]) == "Y": # Switch to enemy mode
	options = ["enemy"]
if (len(sys.argv) > 4 and sys.argv[4] == "Y"): # No Dorm option
	if (len(sys.argv) > 5 and sys.argv[5] == "Y"): # No Back or Dorm option
		options = ["front"]
	options = ["front", "back"]
if (len(sys.argv) > 5 and sys.argv[5] == "Y"): # No Back option
...
for option in options:
	...
	for ft in filetypes:
		...
		localPath = "./assets/char/" + internalname + "/" + option + "/" + buildflag + charname + "." + ft
